...
static void h_drive_input(void) { if(VOIDSTRIKE_HARNESS_SCENARIO_ID==2||VOIDSTRIKE_HARNESS_SCENARIO_ID==3)return;if(VOIDSTRIKE_HARNESS_SCENARIO_ID==1){if(t.frame==0)ana_input_pulse_key_event(ANA_KEY_CTRL);return;}ana_input_pulse_key_event(ANA_KEY_CTRL);ana_input_pulse_key_event(ANA_KEY_RIGHT);if(VOIDSTRIKE_HARNESS_SCENARIO_ID==4&&(t.frame%120)==0)ana_input_pulse_key_event(ANA_KEY_SPACE); }
